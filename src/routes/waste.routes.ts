...
function mapWasteMaterial(row: any) {
  return {
    id: String(row._id),
    code: row.code,
...
wasteRoutes.get("/", requirePagePermission("stock", "view"), async (req, res) => {
  ...
  if (!parsedQuery.success) return fail(res, 400, "Invalid waste query");

  const filter: Record<string, unknown> =
    req.user?.globalRole === "super_admin" ? {} : { factoryId: req.factoryId };
...
wasteRoutes.post("/", requirePagePermission("stock", "add"), async (req, res) => {
  ...
  if (!parsed.success) return fail(res, 400, "Invalid waste payload");
...
wasteRoutes.patch("/:id", requirePagePermission("stock", "update"), async (req, res) => {
  ...
  if (parsed.data.material !== undefined) update.material = parsed.data.material;
  if (parsed.data.projectId !== undefined) update.projectId = parsed.data.projectId;
  if (parsed.data.projectName !== undefined) update.projectName = parsed.data.projectName;
  if (parsed.data.usedForProjectId !== undefined) update.usedForProjectId = parsed.data.usedForProjectId;
  if (parsed.data.usedForProjectName !== undefined) update.usedForProjectName = parsed.data.usedForProjectName;
  ...
  if (!updated) return fail(res, 404, "Waste material not found");
...
wasteRoutes.delete("/:id", requirePagePermission("stock", "delete"), async (req, res) => {
  ...
  if (!deleted) return fail(res, 404, "Waste material not found");
