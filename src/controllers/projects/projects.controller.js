...
import { maybeSendStockNotifications } from "../../services/stock-alerts.service.js";
import { sendProjectLifecycleNotification } from "../../services/project-notifications.service.js";
import { sendProjectInsufficientStockAlert } from "../../services/stock-alerts.service.js";
const projectListQuerySchema = z.object({
    search: z.string().optional().nullable(),
    status: z.enum(["all", "ongoing", "hold", "delivered", "completed", "cancelled"]).optional(),
...
async function notifyProjectPlanningShortages({
    factoryId,
    actorId,
    source,
    projectId = null,
    ...
    if (!factoryId || !Array.isArray(nextMaterials) || nextMaterials.length === 0) {
        return;
    }
    const [allProjects, stockRows] = await Promise.all([
        ProjectModel.find({ factoryId }, { materials: 1, workflowStages: 1 }).lean(),
        StockModel.find({ factoryId }).lean(),
    ]);
    const stockById = new Map(stockRows.map((stock) => [String(stock._id), stock]));
    const currentStockByKey = new Map();
    const usedTotals = new Map();
    const beforeRequired = new Map();
    const afterRequired = new Map();
    const currentProjectTotals = new Map();
    for (const stock of stockRows) {
        const key = resolveMaterialStockKeyFromStock(stock);
        if (!key)
            continue;
    ...
    for (const projectRow of allProjects) {
        ...
        if (projectId && String(projectRow._id) === String(projectId)) {
            continue;
        }
        for (const material of projectRow?.materials ?? []) {
            const key = resolveMaterialStockKeyFromProjectMaterial(material, stockById);
            if (!key)
                continue;
            beforeRequired.set(key, (beforeRequired.get(key) ?? 0) + Number(material.quantity ?? 0));
        }
    }
    ...
    for (const [key, value] of beforeRequired.entries()) {
        afterRequired.set(key, value);
    }
    for (const material of nextMaterials) {
        ...
        if (!key)
            continue;
        const quantity = Number(material.quantity ?? 0);
        afterRequired.set(key, (afterRequired.get(key) ?? 0) + quantity);
        currentProjectTotals.set(key, (currentProjectTotals.get(key) ?? 0) + quantity);
    }
    ...
    for (const material of nextMaterials) {
        ...
        const stockItem = material?.stockItemId && stockById.has(String(material.stockItemId))
            ? stockById.get(String(material.stockItemId))
            : stockRows.find((stock) => resolveMaterialStockKeyFromStock(stock) === key);
        if (!stockItem)
            continue;
        const currentStock = Number(currentStockByKey.get(key) ?? 0);
        const previousTotalRequired = Number(beforeRequired.get(key) ?? 0);
        const nextTotalRequired = Number(afterRequired.get(key) ?? 0);
        const totalUsed = Number(usedTotals.get(key) ?? 0);
        const isInsufficient = currentStock < nextTotalRequired;
        const requirementIncreased = nextTotalRequired > previousTotalRequired;
       
        console.log({ key, currentStock, previousTotalRequired, nextTotalRequired, totalUsed, isInsufficient, requirementIncreased });
        console.log("!requirementIncreased || !isInsufficient", !requirementIncreased || !isInsufficient)
        if (!requirementIncreased || !isInsufficient) {
            continue;
        }
        await sendProjectInsufficientStockAlert({
            factoryId,
            stock: stockItem,
            actorId,
            source,
            shortage: {
                materialName: material.materialName || material.materialType || stockItem.material || stockItem.name,
                unit: material.unit || stockItem.unit || "units",
                currentStock,
                projectRequired: Number(currentProjectTotals.get(key) ?? 0),
                remainingRequired: nextTotalRequired,
                totalRequired: nextTotalRequired,
                totalUsed,
                projectId: projectId ? String(projectId) : "",
                projectName,
            },
        }).catch((error) => console.error("Project planning stock alert failed:", error));
    }
}
...
async function attachStockItemsToProjectMaterials(factoryId, materials = [], actorId, enforceLimits = true) {
    ...
    for (const material of materials) {
        ...
        if (stockItem) {
            previousQuantity = Number(stockItem.quantity ?? 0);
        ...
        void maybeSendStockNotifications({
            factoryId,
            stock: stockItem.toObject ? stockItem.toObject() : stockItem,
            previousQuantity,
            actorId,
            source: "project_new_stock_sync",
        }).catch((error) => console.error("Stock notification send failed:", error));
...
export async function handleListProjects(req, res) {
    try {
        ...
        if (status && status !== "all") {
            filter.status = status === "delivered" || status === "completed"
...
export async function handleAssignProjectToSelf(req, res) {
...
export async function handleUnassignProjectFromSelf(req, res) {
    if (req.user?.globalRole !== "staff") {
...
export async function handleUpdateAssignedStaffStatus(req, res) {
...
export async function handleMarkProjectDelivered(req, res) {
    try {
        ...
        if (!parsed.success)
            return fail(res, 400, "Invalid delivery payload");
        const filter = req.user?.globalRole === "super_admin"
            ? { _id: req.params.id }
        ...
        if (!project)
            return fail(res, 404, "Project not found");
        if (normalizeProjectStatus(project.status) === "delivered") {
            return ok(res, mapProject(project, req.user?.id), "Project already delivered");
        }
        if (req.user?.globalRole === "staff") {
            const user = await UserModel.findOne({ _id: req.user.id, ...projectFilter(req) }, { employeeRole: 1 }).lean();
            const employeeRole = String(user?.employeeRole ?? "").toLowerCase();
            if (!employeeRole.includes("pack") && !employeeRole.includes("deliver")) {
                return fail(res, 403, "Only Packing & Delivery staff can mark projects as delivered");
            }
        }
        project.status = "delivered";
        project.delivery = parsed.data.delivery ? new Date(parsed.data.delivery) : new Date();
        project.updatedBy = req.user?.id;
        await project.save();
        if (req.factoryId) {
            void sendProjectLifecycleNotification({
                factoryId: req.factoryId,
                actorId: req.user?.id,
                eventKey: "projectDelivered",
                project,
            }).catch((error) => console.error("Project delivered notification failed:", error));
        }
        ok(res, mapProject(project, req.user?.id), "Project marked as delivered");
    }
    catch (error) {
...
export async function handleCreateProject(req, res) {
    try {
        ...
        void notifyProjectPlanningShortages({
            factoryId: req.factoryId,
            actorId: req.user?.id,
        ...
        if (req.factoryId) {
            void sendProjectLifecycleNotification({
                ...
                project: created,
            }).catch((error) => console.error("Project created notification failed:", error));
        }
...
export async function handleUpdateProjectWorkflow(req, res) {
...
export async function handleUpdateProjectStageAllocation(req, res) {
    try {
...
export async function handleUpdateProjectStageUsage(req, res) {
    ...
    if (machineKey === "pressing" || machineKey === "cutting") {
        ...
        for (const [stockId, quantity] of stockAdjustments) {
            ...
            currentStockById.set(String(stockId), stock);
        }
        if (stockAdjustments.size) {
            ...
            for (const [stockId, quantity] of stockAdjustments) {
                const previousStock = currentStockById.get(String(stockId));
                if (!previousStock || quantity === 0)
                    continue;
                const updatedStock = await StockModel.findOne({
                    _id: stockId,
                    ...projectFilter(req),
                }).lean();
                if (!updatedStock)
...
export async function handleUpdateProject(req, res) {
    try {
        ...
        if (parsed.data.status !== undefined) {
            update.status = normalizeProjectStatus(parsed.data.status);
        }
        if (parsed.data.delivery) {
            update.delivery = new Date(parsed.data.delivery);
        }
        const nextMaterials = parsed.data.materials
            ? (req.factoryId
                ? await attachStockItemsToProjectMaterials(req.factoryId, parsed.data.materials, req.user?.id, req.user?.globalRole !== "super_admin")
                : parsed.data.materials)
            : current.materials ?? [];
        ...
        if (updated && normalizeProjectStatus(current.status) !== "delivered" && normalizeProjectStatus(updated.status) === "delivered" && req.factoryId) {
            void sendProjectLifecycleNotification({
                factoryId: req.factoryId,
                actorId: req.user?.id,
                eventKey: "projectDelivered",
        ...
        if (parsed.data.materials) {
            void notifyProjectPlanningShortages({
                ...
                projectName: updated?.name || current.name,
                previousMaterials: current.materials ?? [],
