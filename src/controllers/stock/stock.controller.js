...
function stockFilter(req) {
  return req.user?.globalRole === "super_admin"
    ? {}
    : { factoryId: req.factoryId };
}

export async function handleGetStockMaterialTypes(req, res) {
...
export async function handleCreateStockMaterialType(req, res) {
  const parsed = materialTypeSchema.safeParse(req.body);
  if (!parsed.success) return fail(res, 400, "Invalid material type payload");
  ...
  if (!label) return fail(res, 400, "Material type is required");
...
export async function handleListStock(req, res) {
  const parsedQuery = stockListQuerySchema.safeParse(req.query);
  if (!parsedQuery.success) return fail(res, 400, "Invalid stock query");

  const filter = stockFilter(req);
...
export async function handleCreateStock(req, res) {
  try {
    const parsed = stockSchema.safeParse(req.body);
    if (!parsed.success) return fail(res, 400, "Type, thickness, quantity, and unit are required");
    if (!req.factoryId) return fail(res, 400, "Factory scope is required");
    ...
    if (error?.code === 11000) return fail(res, 409, "This stock type and thickness already exists");
    fail(res, 400, error.message || "Unable to create stock");
  }
}

export async function handleUpdateStock(req, res) {
  try {
    ...
    if (!nextType || !nextThickness) return fail(res, 400, "Type and thickness are required");
    ...
    if (parsed.data.thickness !== undefined) update.thickness = parsed.data.thickness;
...
export async function handleUpdateStockQuantity(req, res) {
  ...
  const updated = await StockModel.findOneAndUpdate(
    { _id: req.params.id, ...filter },
    { quantity: parsed.data.quantity, updatedBy: req.user?.id },
    { new: true },
  ).lean();
  ...
  void maybeSendStockNotifications({
    ...
    previousQuantity: current.quantity,
    actorId: req.user?.id,
    source: "stock_quantity_update",
  }).catch((error) => console.error("Stock notification send failed:", error));

  ok(res, mapStockItem(updated), "Stock quantity updated");
}

export async function handleDeleteStock(req, res) {
  const filter = stockFilter(req);
